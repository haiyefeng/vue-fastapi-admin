...
| 独立存在？ | 是 | 否——必须挂在某个待办下 | 是——但通过生成待办参与日常流程 |
| 完成语义 | 勾选一次即完成 | 无完成态，只是时间安排 | 习惯本身永不"完成"；打卡 = 完成它当期生成的待办 |
| 时间属性 | `due_date` 截止时间（可无） | start–end 具体时段，可多段 | 频率规则（每天/每周指定几天/每周N次/隔N天） |
| 出现在哪 | 四象限、任务列表、收件箱 | 日历日/周视图、今日日程 | 习惯页（规则与统计）；生成的待办进今日待办，也可挂时间块上日历 |
| 度量方式 | 完成/未完成 | —（依附于待办） | 连续天数、周期达标率（从习惯待办统计） |

判断规则：做完一次就了结 → 待办；给待办定了具体时段 → 加时间块进日程（同一实体的两面）；无终点、反复坚持、在意连续记录 → 习惯（由系统按频率自动生成每期的待办实例）。

边界约定：

- **不引入独立的"日历事件"模型**。会议等"事件"就是带时间块的待办（如"团队会议 9:00–10:30"），保持单一模型。
- **习惯定期生成待办**。习惯是"待办生成器"：按频率规则自动生成带 `habit_id` 的 TodoItem，勾掉即打卡，统一勾选体验；生成的待办同样可以挂 TimeBlock（如"晨间阅读 每天 7:00–7:30"上日历）。不再需要独立的打卡记录表——习惯待办本身就是打卡历史。
- **手动待办不做"重复规则"**（YAGNI）。周期性事务要么手动建待办，要么建为习惯让系统生成；不给普通待办单独加 recurrence 字段。

## 数据模型全景

```
User ─┬─ TodoItem      已有；一期 +project_id +reminder_at；三期 +goal_id +habit_id
      │    ├─ SubTask   新表（一期）
      │    └─ TimeBlock 新表（二期）
      ├─ Category       已有模型，一期激活
      │    └─ Project   已有模型，一期激活
      ├─ Habit          新表（三期）——"待办生成器"，按频率生成带 habit_id 的 TodoItem
      ├─ Goal           新表（三期）
      └─ Review         新表（四期）
```
...
| `habit_id` | FK → Habit，可空 | 三期 | 非空 = 习惯生成的打卡待办；以 `(habit_id, 所属日期)` 唯一防重复生成 |
...
对应 `habits.html`。习惯是**待办生成器**：按频率规则自动生成带 `habit_id` 的 TodoItem，勾掉生成的待办即打卡。不设独立的打卡记录表——习惯待办本身就是打卡历史，连续天数、达标率均从它统计。
...
| `default_quadrant` | 枚举 QuadrantType | 生成待办的象限归属，默认"重要不紧急" |
| `goal_desc` | CharField，可空 | 如"30分钟"，仅展示 |
| `reminder_time` | time，可空 | 写入生成待办的 `reminder_at`，仅展示 |
| `goal_id` | FK → Goal，可空 | 习惯关联到计划 |
| `is_paused` / `is_archived` | bool | 暂停即停止生成；只归档不物理删除，保留打卡历史 |

**生成机制（关键设计，三期 spec 展开细节）：**

- **读时惰性生成，不引入调度器**。项目无 celery/cron 基础设施；在读取"今天相关"数据的接口（待办列表、今日概览、习惯页）中做幂等检查：当天该生成而未生成的习惯待办即时补上，以 `(habit_id, 所属日期)` 唯一约束防重。
- **定日型 vs 弹性型**。daily / weekly_days / interval_days 为定日型：当天该做就生成，错过的保留为失误记录（计入达标率）。weekly_count 为弹性型：本周额度未满则每天生成一条，额度满即停止；过去几天未完成的弹性待办在下次生成时清理（周三没运动不算错过，周五仍可补）。
- **统计分流**。现有四象限统计、每日完成统计默认排除 `habit_id` 非空的待办（避免自动生成灌水），习惯达标情况作为独立维度展示。
...
- Habit / Goal 模型 + 接口，TodoItem 增加 `habit_id` / `goal_id`，aerich 迁移
- 习惯待办的惰性生成逻辑（幂等、防重、弹性型清理，见 Habit 模型一节）
- 习惯页（参照 `habits.html`）：习惯列表、连续天数、进度条、暂停/归档/恢复、添加习惯弹窗（频率配置 + 默认象限）；页内"今日打卡"按钮 = 完成当天生成的习惯待办
- 现有统计接口分流：四象限统计、每日完成统计默认排除习惯待办
...
- **习惯待办惰性生成的边界**：并发请求可能同时触发生成，靠 `(habit_id, 所属日期)` 唯一约束兜底（冲突时忽略）；"当天"的判定涉及时区，三期 spec 需明确以服务器本地时区还是用户时区为准（当前项目统一用服务器时区，建议沿用）。
